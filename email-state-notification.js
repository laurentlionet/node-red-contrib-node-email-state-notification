...
module.exports = function(RED) {
    ...
    function EmailOutNode(n) {
        ...
        if(n.sendfailedcontrib != null && n.sendfailedcontrib != undefined && n.sendfailedcontrib !=""){
            this.sendfailedcontrib = n.sendfailedcontrib;
        }else{
            this.sendfailedcontrib = "send failed";
        }

        if(n.messagesentcontrib != null && n.messagesentcontrib != undefined && n.messagesentcontrib !=""){
            this.messagesentcontrib = n.messagesentcontrib;
        }else{
            this.messagesentcontrib = "message sent";
        }
                ...
                if (smtpTransport) {
                    ...
                    smtpTransport.sendMail(sendopts, function(error, info) {
                        if (error) {
                            ...
                            node.status({fill:"red",shape:"ring",text:"email.status.sendfailedcontrib",response:error.response,msg:{to:msg.to,topic:msg.topic,id:msg._msgid}});
                            node.send({payload:{state:node.sendfailedcontrib,email:sendopts,error:error}});
                        } else {
                            node.log(RED._("email.status.messagesent",{response:info.response}));
                            node.status({fill:"green",shape:"dot",text:"email.status.messagesentcontrib",response:info.response,msg:{to:msg.to,topic:msg.topic,id:msg._msgid}});
                            node.send({payload:{state:node.messagesentcontrib,email:sendopts,info:info}});
                            if (done) { done(); }
                        }
                    });
                }
    ...
    RED.nodes.registerType("email out",EmailOutNode,{
        credentials: {
            userid: {type:"text"},
    ...
    RED.nodes.registerType("email in",EmailInNode,{
        credentials: {
            userid: { type:"text" },
            password: { type: "password" },
            global: { type:"boolean" }
